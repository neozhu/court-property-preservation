      ...
      if (users.updated != null)
      {
        foreach (var item in users.updated)
        {
          ...
          if (result.Succeeded)
          {
            ...
            this.UserManager.AddClaim(user.Id, new System.Security.Claims.Claim("http:            this.UserManager.AddClaim(user.Id, new System.Security.Claims.Claim("FullName", string.IsNullOrEmpty(user.FullName) ? "" : user.FullName));
