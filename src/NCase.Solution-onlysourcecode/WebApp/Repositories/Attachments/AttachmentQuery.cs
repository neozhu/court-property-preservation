...
/// Created Date: 3/3/2020 1:33:16 PM
/// Author: neo.zhu
		...
		public AttachmentQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "DocId"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.DocId.Contains(rule.value));
						}
						...
						if (rule.field == "ExpireDate" && !string.IsNullOrEmpty(rule.value) )
						{	
							if (rule.op == "between")
                            {
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.ExpireDate) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.ExpireDate) <= 0);
						    }
						}
