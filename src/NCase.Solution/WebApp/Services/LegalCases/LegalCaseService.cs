...
{
  /// <summary>
  /// File: LegalCaseService.cs
  /// Purpose: Within the service layer, you define and implement 
  /// the service interface and the data contracts (or message types).
  /// One of the more important concepts to keep in mind is that a service
  /// should never expose details of the internal processes or 
  /// the business entities used within the application. 
  /// Created Date: 3/4/2020 9:28:10 AM
  /// Author: neo.zhu
  ...
  public class LegalCaseService : Service<LegalCase>, ILegalCaseService
  {
    ...
    public async Task Register(LegalCase item) {
      //var caseid=await this.GetCaseId($"（{DateTime.Now.Year}）", item.SerialNumber, item.Category);
      item.Subject = $"{item.Accuser}，{item.Defendant}";
      item.Project = item.CaseId;
      //item.CaseId = caseid.Item1;
    ...
    public async Task Assigning(AssigningViewModel assigning,string user) {
      ...
      foreach (var item in list)
      {
        ...
        item.ToDoDate = now;
        item.ToDepartment = assigning.ToDepartment;
        //item.PreCloseDate = assigning.PreCloseDate;
        //item.Opinion1 = assigning.Opinion1;
        //item.Opinion2 = assigning.Opinion2;
        item.ToUser = assigning.ToUser;
        item.Recorder = assigning.Recorder;
        item.AllocateDate = assigning.AllocateDate;
        item.Court = assigning.ToDepartment;
    ...
    public async Task AssigningTask(LegalCase legalcase,string user){
      ...
      item.Status = nodeconfig.NextStatus;
      item.ToDoDate = now;
      //item.PreCloseDate = legalcase.PreCloseDate;
      ...
      //item.Opinion2 = legalcase.Opinion2;
      //item.Opinion1 = legalcase.Opinion1;
    ...
    public async Task<Tuple<string,int,int>> GetCaseId(string year, string serial, string category) {
      ...
      //var maxid =await this.Queryable().Where(x => x.CaseId.Contains(prefix)).MaxAsync(x=>x.Id);
      //var count = maxid;
      var delcaseid =true;//await this.getdelcaseid(year, category, serial);
      if (delcaseid)
      {
        //var num = KeyGenerator.GetNextCaseId(prefix);
        var expires = await this.nodeTimeService.Queryable()
...
order by  CONVERT(int, SUBSTRING(caseid, 14, Len(CaseId) - 14)) desc";
      ...
      //else
      //{
      //  var tuple = new Tuple<string, int, int>(delcaseid.CaseId, 1, delcaseid.Expires);
      //  this.tempCaseIdService.Delete(delcaseid);
      //  return tuple;
      //}
