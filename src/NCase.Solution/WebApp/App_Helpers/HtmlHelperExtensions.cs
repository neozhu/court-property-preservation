...
namespace WebApp
{
  public static class HtmlHelperExtensions
  {
    ...
    public static HtmlString MenuBadge(this HtmlHelper helper, string name,string user="")
    {
      var count = 0;
      var tag = "";
      if (name == "查询")
      ...
      {
        count = Dashboard.Instance.S1Count();
        tag = $"<span class='badge bg-color-greenLight pull-right inbox-badge'>{count}</span>";
      }
      else if (name == "分案处理")
      {
        count = Dashboard.Instance.S0Count();
        tag = $"<span class='badge bg-color-greenLight pull-right inbox-badge'>{count}</span>";
      }
      else if (name == "案件执行" || name == "办案处理")
      {
        count = Dashboard.Instance.S2Count(user);
        tag = $"<span class='badge bg-color-greenLight pull-right inbox-badge'>{count}</span>";
      }
      else if (name == "案件审批")
      {
        count = Dashboard.Instance.S6Count(user);
        tag = $"<span class='badge bg-color-greenLight pull-right inbox-badge'>{count}</span>";
      }
      //else if (name == "结案归档")
      //{
      //  count = Dashboard.Instance.S5Count();
      //  tag = $"<span class='badge bg-color-greenLight pull-right inbox-badge'>{count}</span>";
      //}
      else
      {
